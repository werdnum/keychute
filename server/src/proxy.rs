...
/// The grant id from the raw request path.
///
/// Parsed here rather than by a `Path<Uuid>` extractor so that a malformed id
/// fails INSIDE the handler and therefore through [`ApiFailure`], which is what
/// stamps [`KEYCHUTE_ERROR_HEADER`]. An extractor rejection is generated by
/// axum before any handler runs and carries no marker, so a client following
/// the documented header contract would read Keychute's own 400 as an upstream
/// response — on a route where no upstream call happened at all.
///
/// A non-uuid id is `NotFound` for the same reason every other grant route
/// uses it: it is certainly not a grant this client owns, and the answer must
/// not distinguish "malformed" from "someone else's".
fn grant_id_from_path(raw_path: &str) -> Result<Uuid, ApiFailure> {
    let after_prefix = raw_path
        .strip_prefix("/v1/grants/")
        .ok_or(ApiFailure::NotFound)?;
    let (id, _rest) = after_prefix.split_once('/').ok_or(ApiFailure::NotFound)?;
    id.parse::<Uuid>().map_err(|_| ApiFailure::NotFound)
}
...
pub async fn proxy_root(
    ...
    let id = grant_id_from_path(req.uri().path())?;
