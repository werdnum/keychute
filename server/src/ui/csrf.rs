...
//!    `route \0 action_id \0 subject \0 expiry_decimal`. Single-purpose —
//!    a token for one route/object never validates for another. Max age 15 min.
...
fn mac_input(route: &str, action_id: &str, subject: &str, expiry_unix: i64) -> Vec<u8> {
...
/// Mint a form token bound to (route, action id, subject), valid 15 minutes.
pub fn issue_token(
    keyset: &Keyset,
    route: &str,
    action_id: &str,
    ...
    let mac = keyset.csrf_mac(&mac_input(route, action_id, subject, expiry));
...
pub fn verify_token(
    ...
    let expected = keyset.csrf_mac(&mac_input(route, action_id, subject, expiry));
...
mod tests {
    ...
    fn csrf_rejects_garbage_and_tampering() {
        ...
        raw[12] ^= 1;
        assert!(!verify_token(&ks, "r", "a", "s", &B64.encode(&raw), now));
